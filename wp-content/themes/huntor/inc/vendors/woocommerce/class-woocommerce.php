...
if (!class_exists('huntor_WooCommerce')) :


    class huntor_WooCommerce {
        ...
        /**
         * @return huntor_WooCommerce
         */
        public static function getInstance() {
            if (!isset(self::$instance) && !(self::$instance instanceof huntor_WooCommerce)) {
                self::$instance = new huntor_WooCommerce();
            }
        ...
        public function __construct() {
            ...
            add_filter('huntor_customizer_buttons', array($this, 'customizer_buttons'));
            ...
            if (class_exists('YITH_WCWL')) {
                add_action('wp_ajax_huntor_update_wishlist_count', array($this, 'yith_wcwl_ajax_update_count'));
                add_action('wp_ajax_nopriv_huntor_update_wishlist_count', array(
                    $this,
            ...
            // Woocommerce 3.3
            if (huntor_woocommerce_version_check('3.3')) {
...
//                add_action('woocommerce_after_shop_loop', 'woocommerce_result_count', 5);
            ...
            add_action('wp_ajax_huntor_product_view', array($this, 'huntor_ajax_wc_track_product_view'));
            add_action('wp_ajax_nopriv_huntor_product_view', array($this, 'huntor_ajax_wc_track_product_view'));
        }

        function huntor_ajax_wc_track_product_view() {
        ...
        public function mobile_handheld_footer_bar() {
            $links = array(
                'my-account' => '<a class="my-accrount-footer" href="' . esc_url(get_permalink(get_option('woocommerce_myaccount_page_id'))) . '">' . esc_attr__('My Account', 'huntor') . '</a>',
                'search'     => '<a class="search-footer" href="">' . esc_attr__('Search', 'huntor') . '</a><div class="site-search">' . get_search_form(false) . '</div>',
                'cart'       => ' <a class="footer-cart-contents" href="' . esc_url(wc_get_cart_url()) . '" title="' . esc_attr__('View your shopping cart', 'huntor') . '">  <span class="count">''</span></a>'
            );
            ...
            if (function_exists('wvs_get_wc_attribute_taxonomy')) {
                ...
                switch ($attr->attribute_type) {
                    case 'color':
                        ...
                        $html  .= '<span class="color-label" style="background: ' . wp_kses_post($color) . ';"></span>';
                        $html  .= '<span class="color-name">' . wp_kses_post($term->name) . '</span>';
                        $html  .= '<span class="color-count">' . wp_kses_post($count) . '</span>';
                        $html  .= '</a>';
                    ...
                    case 'button':
                        ...
                        $html  .= '<span class="attr-label">' . wp_kses_post($term->name) . '</span>';
                        $html  .= '</a>';
        ...
        public function edit_section_customizer($wp_customizer) {
            $wp_customizer->get_control('woocommerce_single_image_width')->section  = 'huntor_woocommerce_single';
            $wp_customizer->get_control('woocommerce_single_image_width')->priority = 9;

            $wp_customizer->get_control('woocommerce_thumbnail_image_width')->section = 'huntor_woocommerce_product';
            $wp_customizer->get_control('woocommerce_thumbnail_cropping')->section    = 'huntor_woocommerce_product';

            $wp_customizer->get_control('woocommerce_shop_page_display')->section  = 'huntor_woocommerce_archive';
            $wp_customizer->get_control('woocommerce_shop_page_display')->priority = 21;

            $wp_customizer->get_control('woocommerce_category_archive_display')->section  = 'huntor_woocommerce_archive';
            $wp_customizer->get_control('woocommerce_category_archive_display')->priority = 21;

            $wp_customizer->get_control('woocommerce_default_catalog_orderby')->section  = 'huntor_woocommerce_archive';
        ...
        public function label_tooltip() {
            ...
            echo '<div id="osf-woocommerce-cart">' . esc_html__('Add to cart', 'huntor') . '</div>';
            echo '</div>';
        }
        ...
        public function added_to_cart_template() {
            $text = esc_html__('has been added to your cart', 'huntor');
            echo <<<HTML
...
HTML;
        ...
        public function widgets_init() {
            register_sidebar(array(
                'name'          => esc_html__('WooCommerce Shop', 'huntor'),
            ...
            register_sidebar(array(
                'name'          => esc_html__('WooCommerce Detail', 'huntor'),
        ...
        public function render_woocommerce_shop_canvas() {
            ...
            $position = get_theme_mod('huntor_woocommerce_archive_layout', '2cl');
            if (is_active_sidebar('sidebar-woocommerce-shop')) {
                echo '<div id="opal-canvas-filter" class="opal-canvas-filter position-' . wp_kses_post($position) . '"><span class="filter-close">' . esc_html__('CLOSE', 'huntor') . '</span><div class="opal-canvas-filter-wrap">';
        ...
        public function render_button_shop_canvas() {
            if (is_active_sidebar('sidebar-woocommerce-shop')) {
                echo '<button class="filter-toggle" aria-expanded="false"><span class="filter-icon"></span>' . esc_html__('Filter', 'huntor') . '</button>';
            }
        }
            ...
            if (!empty($query)) {
                ...
                if (is_object($product_object)) {
                    ...
                    if (!empty($product_sku)) {
                        $product_sku_display = ' - ' . esc_html__('Sku', 'huntor') . ': ' . wp_kses_post($product_sku);
                    }

                    $product_title_display = '';
                    if (!empty($product_title)) {
                        $product_title_display = ' - ' . esc_html__('Title', 'huntor') . ': ' . wp_kses_post($product_title);
                    }

                    $product_id_display = esc_html__('Id', 'huntor') . ': ' . wp_kses_post($product_id);
        ...
        public function productCategoryAutocompleteRender($query) {
            ...
            $term_slug_display = '';
            if (!empty($term_slug)) {
                $term_slug_display = ' - ' . esc_html__('Sku', 'huntor') . ': ' . $term_slug; // WPCS: XSS ok.
            }

            $term_title_display = '';
            if (!empty($term_title)) {
                $term_title_display = ' - ' . esc_html__('Title', 'huntor') . ': ' . $term_title; // WPCS: XSS ok.
            }

            $term_id_display = esc_html__('Id', 'huntor') . ': ' . $term_id; // WPCS: XSS ok.
        ...
        public function taxonomy_metaboxes() {
            ...
            $cmb_term = new_cmb2_box(array(
                'id'           => 'product_cat',
                'title'        => esc_html__('Product Metabox', 'huntor'), // Doesn't output for term boxes
            ...
            $cmb_term->add_field(array(
                'name'       => esc_html__('Banner', 'huntor'),
                //                'desc' => esc_html__('Location image', 'huntor'),
                'id'         => $prefix . 'banner',
                'type'       => 'file',
                'options'    => array(
        ...
        public function product_video_custom_field() {
            $prefix = 'huntor_products_';
            $cmb    = new_cmb2_box(array(
                'id'           => $prefix . 'product_video',
                'title'        => esc_html__('Product Video Config', 'huntor'),
            ...
            $cmb->add_field(array(
                'name' => esc_html__('Product video', 'huntor'),
                'desc' => esc_html__('Supports video from youtube and vimeo.', 'huntor'),
                'id'   => $prefix . 'video',
                'type' => 'oembed',
            ));

            $cmb->add_field(array(
                'name'         => esc_html__('Video Thumbnail', 'huntor'),
            ...
            if (isset($atts['product_layout']) && $atts['product_layout'] != 'grid') {
                ...
                if ($atts['product_layout'] === 'list' || $atts['product_layout'] === 'list-carousel') {
                    if (!empty($atts['show_category'])) {
                        add_action('huntor_product_list_before_title', 'huntor_woocommerce_get_product_category', 10);
                    }

                    if (!empty($atts['show_rating'])) {
                        add_action('huntor_product_list_before_price', 'huntor_woocommerce_list_show_rating', 15);
                    }

                    if (!empty($atts['show_except'])) {
                        add_action('huntor_product_list_before_price', 'huntor_woocommerce_list_show_excerpt', 10);
                    }

                    if (!empty($atts['show_time_sale'])) {
                        add_action('huntor_product_list_after_price', 'huntor_woocommerce_time_sale', 20);
                    }
        ...
        public function style_loop_end($atts = array()) {
            if (isset($atts['product_layout']) && $atts['product_layout'] != 'grid') {
                if ($atts['product_layout'] === 'list') {
                    echo '</div>';
                    if (!empty($atts['show_category'])) {
                        remove_action('huntor_product_list_before_title', 'huntor_woocommerce_get_product_category', 10);
                    }

                    if (!empty($atts['show_rating'])) {
                        remove_action('huntor_product_list_before_price', 'huntor_woocommerce_list_show_rating', 15);
                    }

                    if (!empty($atts['show_except'])) {
                        remove_action('huntor_product_list_before_price', 'huntor_woocommerce_list_show_excerpt', 10);
                    }

                    if (!empty($atts['show_time_sale'])) {
                        remove_action('huntor_product_list_after_price', 'huntor_woocommerce_time_sale', 20);
                    }
                }
            }
        }
        ...
        public function body_class($classes) {
            ...
            if (huntor_is_product_archive()) {
                ...
                if (get_theme_mod('huntor_woocommerce_archive_style', 'default') == 'default') {
                    $classes[] = 'opal-woocommerce-archive-style-default';
                    if (is_active_sidebar('sidebar-woocommerce-shop') && get_theme_mod('huntor_woocommerce_archive_layout') !== '2cr') {
                ...
                if (get_theme_mod('huntor_woocommerce_archive_product_width', 0)) {
                    $classes[] = 'huntor_woocommerce_archive_product_style_full';
                }
            } else {
                if (is_product()) {
                    $classes = array_diff($classes, array(
                    ...
                    if (is_active_sidebar('sidebar-woocommerce-detail') && get_theme_mod('huntor_woocommerce_single_layout') !== '2cl') {
                        $classes[] = 'opal-content-layout-2cr';
                    }
                    ...
                    $classes[] = 'woocommerce-single-style-' . get_theme_mod('huntor_woocommerce_single_product_style', '1');

                }
            }

            $classes[] = 'product-style-' . get_theme_mod('huntor_woocommerce_product_style', 1);

            return $classes;
        }

        public function set_sidebar($name) {
            if (huntor_is_product_archive() && is_active_sidebar('sidebar-woocommerce-shop') && (get_theme_mod('huntor_woocommerce_archive_style', 'default') == 'default')) {
        ...
        public function related_products_args($args) {
            $args = apply_filters('huntor_related_products_args', array(
                'posts_per_page' => get_theme_mod('huntor_woocommerce_single_related_number', 3),
                'columns'        => get_theme_mod('huntor_woocommerce_single_related_columns', 3),
            ));
        ...
        public function thumbnail_columns() {
            $columns = get_theme_mod('huntor_woocommerce_product_thumbnail_columns', 3);

            return intval(apply_filters('huntor_product_thumbnail_columns', $columns));
        }
        ...
        public function products_per_page() {
            $number = get_theme_mod('huntor_woocommerce_archive_number', 12);

            return intval(apply_filters('huntor_products_per_page', $number));
        }
        ...
        public function customizer_buttons($buttons) {
            $buttons = wp_parse_args($buttons, array(
                ...
                '.woocommerce-pagination'              => array(
                    array(
                        'id'      => 'huntor_layout_pagination_style',
                        'icon'    => 'default',
                        'type'    => 'control',
                ...
                '.single-product .flex-control-thumbs' => array(
                    array(
                        'id'      => 'huntor_woocommerce_product_thumbnail_columns',
                        'icon'    => 'default',
                        'type'    => 'control',
                ...
                '.single-product .related'             => array(
                    array(
                        'id'      => 'huntor_woocommerce_single_related_columns',
                        'icon'    => 'default',
                        'type'    => 'control',
                        'trigger' => 'select|focus',
                    ),
                ...
                '.single-product .upsells'             => array(
                    array(
                        'id'      => 'huntor_woocommerce_single_upsale_columns',
                        'icon'    => 'default',
                        'type'    => 'control',
                        'trigger' => 'select|focus',
                    ),
                ...
                '.products .type-product'              => array(
                    array(
                        'id'      => 'huntor_woocommerce_product_hover',
                        'icon'    => 'default',
                        'type'    => 'control',
                        'trigger' => 'select|focus',
                    ),
                ...
                '#osf-accordion-container'             => array(
                    array(
                        'id'      => 'huntor_woocommerce_single_product_tab_style',
                        'icon'    => 'layout',
                        'type'    => 'control',
                        'trigger' => 'select|focus',
                    ),
        ...
        public function custom_comment_form($comment_form) {
            ...
            $comment_form['fields']        = array(
                'author' => '<p class="comment-form-author"><input id="author" name="author" type="text" placeholder="' . esc_attr__("Name", "huntor") . '" value="' . esc_attr($commenter['comment_author']) . '" size="30" required /></p>',
                'email'  => '<p class="comment-form-email"><input id="email" name="email" type="email" placeholder="' . esc_attr__("Email", "huntor") . '" value="' . esc_attr($commenter['comment_author_email']) . '" size="30" required /></p>',
                'url'    => '<p class="comment-form-url"> <input id="url" name="url" type="url" placeholder="' . esc_attr__("Website", "huntor") . '" value="' . esc_attr($commenter['comment_author_url']) . '" size="30" maxlength="200" /></p>'
            );
            $comment_form['comment_field'] = '';

            if (wc_review_ratings_enabled()) {
                $comment_form['comment_field'] = '<div class="comment-form-rating"><label for="rating">' . esc_html__('Your rating', 'huntor') . '</label><select name="rating" id="rating" required>
						<option value="">' . esc_html__('Rate&hellip;', 'huntor') . '</option>
						<option value="5">' . esc_html__('Perfect', 'huntor') . '</option>
						<option value="4">' . esc_html__('Good', 'huntor') . '</option>
						<option value="3">' . esc_html__('Average', 'huntor') . '</option>
						<option value="2">' . esc_html__('Not that bad', 'huntor') . '</option>
						<option value="1">' . esc_html__('Very poor', 'huntor') . '</option>
					</select></div>';
            }


            $comment_form['comment_field'] .= '<p class="comment-form-comment"><textarea id="comment" name="comment" cols="45" rows="2" required placeholder="' . esc_attr__("Your review", "huntor") . '"></textarea></p>';
            $comment_form['submit_button'] = '<button name="%1$s" type="submit" id="%2$s" class="%3$s"><span>%4$s</span></button>';
            $comment_form['label_submit']  = esc_html__('Submit Your Review', 'huntor');
...
huntor_WooCommerce::getInstance();