...
class Huntor_Theme_Admin {
	...
	public function edit_menu_title_plugin( $args ) {
		...
		if ( $count > 0 ) {
			$args['menu_title'] = esc_html__( 'Install Plugins', 'huntor' ) . ' <span class="update-plugins"><span class="update-count">' . wp_kses_post( $count ) . '</span></span>';
		}
	...
	public function dashboard_scrips( $hook ) {
		...
		if ( in_array( get_current_screen()->base, $listPage ) ) {
			...
			wp_enqueue_style( 'huntor_dashboard_css', get_template_directory_uri() . '/inc/admin/css/admin.css' );
		}
	...
	public function add_license_page() {
		...
		$cmb = new_cmb2_box( array(
			...
			'title'        => esc_html__( 'License', 'huntor' ),
			'object_types' => array( 'options-page' ),
			'option_key'   => 'lexus-theme-license',
			'parent_slug'  => 'lexus-theme',
		) );
		$cmb->add_field( array(
			'name'    => esc_html__( 'User Name Envato', 'huntor' ),
			'id'      => 'username',
			'type'    => 'text',
			'default' => '',
		) );
		...
		$cmb->add_field( array(
			'name'    => esc_html__( 'Purchase Code', 'huntor' ),
			'id'      => 'purchased_code',
			'type'    => 'text',
			'default' => '',
		) );
	...
	public function edit_tgmpa_notice_action_links( $action_links ) {
		...
		if ( 'lexus-theme-plugins' == $current_screen->id ) {
			$link_template = '<a id="manage-plugins" class="button-primary" style="margin-top:1em;" href="#opal-install-plugins">' . esc_attr__( 'Manage Plugins Below', 'huntor' ) . '</a>';
		} else {
			$link_template = '<a id="manage-plugins" class="button-primary" style="margin-top:1em;" href="' . esc_url( self_admin_url( 'admin.php?page=lexus-theme-plugins' ) ) . '#opal-install-plugins">' . esc_attr__( 'Go Manage Plugins', 'huntor' ) . '</a>';
		}
...
//
	...
	public function edit_admin_menus() {
		...
		if ( current_user_can( 'edit_theme_options' ) ) {
			$submenu['lexus-theme'][0][0] = esc_attr__( 'Welcome', 'huntor' );
		}
	}
	...
	public function custom_menu_import( $args ) {
		...
		$args['menu_title']  = esc_html__( 'Import Demos', 'huntor' );
	...
	private function setup_tab_menus() {
		...
		if ( $this->get_plugins_require_count() <= 0 ) {
			$this->tabs_menu['demos'] = [
				'name' => esc_html__( 'Demo Importer', 'huntor' ),
		...
		$this->tabs_menu['tutorials'] = [
			'name' => esc_html__( 'Tutorials', 'huntor' ),
                ...
                <div class="row">
                    <div class="col">
                        <div class="inner theme-infomation">
                            ...
                            <h1 class="theme-title"><?php esc_attr_e( 'Welcome to', 'huntor' ); ?>

								<?php echo esc_html( $my_theme->get( 'Name' ) ); ?> !
                            </h1>
                            <span class="theme-version"><?php echo esc_attr__( 'Version', 'huntor' ) . ' ' . $my_theme->get( 'Version' );// WPCS: XSS ok.  ?></span>
                        </div>
                        <ul class="opal-tabs">
...
JS;
	...
	public static function getInstance() {
		if ( ! isset( self::$instance ) && ! ( self::$instance instanceof Huntor_Theme_Admin ) ) {
			self::$instance = new Huntor_Theme_Admin();
		}
...
Huntor_Theme_Admin::getInstance();